...
function rangeFromToMonth(from?: MonthStr, to?: MonthStr): { from?: string; to?: string } {
...
export default function ReportsPage() {
  ...
  const drawLucyFooter = (doc: PDF) => {
    ...
    doc.text("Relatorio gerado automaticamente pela Lucy", 14, h - 10);
  ...
  const buildPdf = () => {
    ...
    autoTable(doc, {
      ...
      headStyles: { fillColor: [109, 40, 217], textColor: 255, halign: "center" },
  ...
  const handleSendEmail = async () => {
    const shared = await trySharePdf("Relatorio Financeiro", `Relatorio financeiro de ${userName}`);
    if (!shared) {
      downloadPdf();
      const assunto = encodeURIComponent("Relatorio Financeiro");
      const corpo = encodeURIComponent(
        `O relatorio financeiro de ${userName} foi baixado no seu dispositivo.\nAnexe o arquivo ${pdfFilename()} ao enviar este e-mail.\n`
      );
  ...
  const handleSendWhatsApp = async () => {
    ...
    if (!shared) {
      ...
      const texto = encodeURIComponent(
        `Relatorio financeiro de ${userName}. O PDF foi baixado no seu dispositivo. Anexe o arquivo ${pdfFilename()} na conversa.`
      );
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* Filtro de período */}
      <div className="flex flex-col items-center justify-center bg-white p-6 rounded-2xl shadow-lg border border-purple-100">
        <div className="w-full max-w-3xl flex flex-col sm:flex-row items-center gap-4">
          <div className="flex-1 w-full">
            <label className="text-sm font-semibold text-gray-700 block mb-1">De (mês)</label>
            <input
              type="month"
              value={mesDe}
          ...
          <div className="flex-1 w-full">
            <label className="text-sm font-semibold text-gray-700 block mb-1">Até (mês)</label>
            <input
              type="month"
              value={mesAte}
        ...
        <p className="text-sm text-gray-500 mt-3">
          Dica: Preencha só <b>“De”</b> para um mês específico. Preencha <b>“De”</b> e <b>“Até”</b> para um intervalo.
        </p>
      </div>
